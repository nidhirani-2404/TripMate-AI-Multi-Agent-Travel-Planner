...
def _llm_text(system_prompt: str, user_prompt: str, max_retries: int = 6) -> str:
    time.sleep(0.5)
    for attempt in range(max_retries):
        try:
            ...
            err_str = str(exc)
            if ("429" in err_str or "rate_limit" in err_str or "tokens" in err_str) and attempt < max_retries - 1:
                wait_time = 2.5 * (attempt + 1)
                print(f"Rate limited by Groq API (429). Retrying in {wait_time}s (attempt {attempt + 1}/{max_retries})...", flush=True)
...
"""
    ...
    return {
        ...
        "messages": [AIMessage(content=final_data)],
        "llm_calls": state.get("llm_calls", 0) + 1,
    }
